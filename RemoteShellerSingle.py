# -*- coding: utf-8 -*-
# @Time    : 2025/10/16 16:31:27
# @Author:  yuansky9420
# @Intsall : pip install paramiko requests pyyaml
# @Name    : RemoteSheller 单文件版
# @Description: 这是单文件版，在脚本内部配置任务并执行
# @Version: 1.0.0 alpha
...
TASKS: List[Dict[str, Any]] = [
    ...
    {
        "name": "停止BCS容器",
        "cmd": "docker-compose -f /app/docker-compose-bcs.yml down",
    },
]
...
def probe_exec(item: Dict[str, Any], default_timeout: int = 5) -> bool:
    ...
    if probe_type == "http":
        ...
        raise ValueError(f"未知的探活类型: {probe_type}")
...
def handle_post_execution(task: dict) -> None:
    ...
    if "probes" in task and task["probes"]:
        ...
        for attempt in range(1, retries + 1):
            print(
                f"【信息】 尝试 {attempt}/{retries}: 探活 {len(probes)} 个端点...")
            all_ok = True
            for p in probes:
